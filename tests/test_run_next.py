...
class RunNextTest(unittest.TestCase):
    ...
    def test_checkpoint_allowlist_includes_generated_state_and_block_translations(self):
        ...
        self.assertFalse(run_next.allowed_change("reviews/mastering/0009/state.json", 9))
    ...
    def test_accepted_resume_skips_models_and_commits(self):
        with tempfile.TemporaryDirectory() as directory:
            state_path = Path(directory) / "workflow.json"
            state_path.write_text('{"stage":"ACCEPTED"}\n', encoding="utf-8")
            lock = SimpleNamespace(update=lambda **_values: None)
            with (
                ...
                patch.object(run_next, "run_to_accepted") as workflow,
                patch.object(run_next, "print_cost_report"),
                patch.object(run_next, "commit_accepted") as commit,
            ):
                self.assertEqual(run_next.main(), 0)
        workflow.assert_not_called()
        commit.assert_called_once_with(14)

    def test_mastered_in_flight_redirects_to_mastering_runner(self):
        with tempfile.TemporaryDirectory() as directory:
            state_path = Path(directory) / "workflow.json"
            ...
            with (
                ...
                with self.assertRaisesRegex(SystemExit, "run_next_mastering"):
                    run_next.main()
    ...
    def test_workflow_command_rejects_master(self):
        with self.assertRaisesRegex(SystemExit, "forbidden automatic action"):
            run_next.run_workflow_command(14, "python tools/workflow.py master 14")
