...
SOL paragraphs: P26
Korean lines: 53
Terminology alert: `경험치` → `EXP` present in BASE, absent from SOL.
...
SOL paragraphs: P132
Korean lines: 283
Terminology alert: `퀘스트` → `Quest` present in BASE, absent from SOL.
...
SOL paragraphs: P137
Korean lines: 293
Terminology alert: `문주` → `Sect Leader` present in BASE, absent from SOL.
Terminology alert: `문주님` → `Sect Leader` present in BASE, absent from SOL.
...
SOL paragraphs: P173
Korean lines: 369
Terminology alert: `내상` → `Internal Injury` present in BASE, absent from SOL.
...
Return exactly the JSON object required by the adjudicator brief, with chapter set to 120 and exactly one decision for each of the 99 hunks.
