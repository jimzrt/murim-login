# Mastering Diff — 105 changed hunks

## H001 (replace)

Baseline paragraphs: P3
SOL paragraphs: P3
Korean lines: 7

BASE:

The red eyes blinked, almost tame. So even a C-rank Rare Monster—the terrifying Hobgoblin Great Warrior—could make a face like that.

SOL:

The red eyes blinked, almost docile. So even a C-rank Rare Monster—the terrifying Hobgoblin Great Warrior—could make a face like that.

## H002 (replace)

Baseline paragraphs: P4
SOL paragraphs: P4
Korean lines: 9

BASE:

*Why didn’t I realize it until now?*

SOL:

*Why hadn’t I noticed before?*

## H003 (replace)

Baseline paragraphs: P5
SOL paragraphs: P5
Korean lines: 11

BASE:

With that passing thought, I shook the blood off the spearhead. At the same time—

SOL:

With that passing thought, I shook the blood from my spearhead. At the same moment—

## H004 (replace)

Baseline paragraphs: P7
SOL paragraphs: P7
Korean lines: 15

BASE:

Something heavy hit the ground. The Great Warrior saw what it was and, without meaning to, shuffled backward.

SOL:

Something heavy hit the ground. The Great Warrior saw what it was and instinctively shuffled backward.

## H005 (replace)

Baseline paragraphs: P8
SOL paragraphs: P8
Korean lines: 17

BASE:

Well, fair enough. Its right arm had been snipped clean off below the elbow.

SOL:

Well, fair enough. Its right arm had been severed cleanly below the elbow.

## H006 (replace)

Baseline paragraphs: P10
SOL paragraphs: P10
Korean lines: 21

BASE:

Jin Family’s Manoeuvre Technique. Yeah—I’d used that exact technique to drive in against its chest. The Great Warrior reflexively swung its right arm, but that space was already empty.

SOL:

The Jin Family’s Manoeuvre Technique. Yeah, I’d used it to slip inside the creature’s reach. The Great Warrior reflexively swung its right arm, but there was nothing left to swing.

## H007 (replace)

Baseline paragraphs: P12
SOL paragraphs: P12
Korean lines: 25

BASE:

I set the spearhead against the Great Warrior’s side and drove it upward in the same motion. Charged with internal energy, the blade split hard skin and muscle.

SOL:

I pressed the spearhead against the Great Warrior’s side and ripped it upward. The blade, charged with internal energy, split its tough hide and muscle.

## H008 (replace)

Baseline paragraphs: P14
SOL paragraphs: P14
Korean lines: 29

BASE:

Its left arm came off in a spray of green blood.

SOL:

Its left arm fell away in a spray of green blood.

## H009 (replace)

Baseline paragraphs: P15
SOL paragraphs: P15
Korean lines: 31

BASE:

Both arms gone in an instant, the Hobgoblin Great Warrior bellowed, the sound smeared with rage and pain.

SOL:

Both arms gone in an instant, the Hobgoblin Great Warrior let out a roar of rage and pain.

## H010 (replace)

Baseline paragraphs: P17
SOL paragraphs: P17
Korean lines: 35

BASE:

But I still couldn’t let my guard down. Even without arms, its body alone was still more than dangerous enough.

SOL:

But I couldn’t let my guard down. Even without arms, it was still more than dangerous enough with its body alone.

## H011 (replace)

Baseline paragraphs: P18
SOL paragraphs: P18
Korean lines: 37

BASE:

“Yeah. Come on in.”

SOL:

“Yeah. Come at me.”

## H012 (replace)

Baseline paragraphs: P19
SOL paragraphs: P19
Korean lines: 39

BASE:

Before I’d even finished, that massive frame charged.

SOL:

Before I’d even finished speaking, its massive frame charged.

## H013 (replace)

Baseline paragraphs: P21
SOL paragraphs: P21
Korean lines: 43

BASE:

As expected, this thing’s body was a weapon all by itself. True to being a monster, it had primitive senses and strength beyond any human.

SOL:

As expected, this thing’s entire body was a weapon. Like the monster it was, it possessed primal senses and strength that far surpassed a human’s.

## H014 (replace)

Baseline paragraphs: P23
SOL paragraphs: P23
Korean lines: 47

BASE:

Hooked claws grazed my forearm. A handful of flesh tore away, and blood poured out. Without flinching, I blocked the heel stomping down at the crown of my head.

SOL:

A hooked claw grazed my forearm. A chunk of flesh tore away, and blood poured out. Without flinching, I blocked the heel descending toward the crown of my head.

## H015 (replace)

Baseline paragraphs: P25
SOL paragraphs: P25
Korean lines: 51

BASE:

The ground under my feet started to sink.

SOL:

The ground beneath my feet began to sink.

## H016 (replace)

Baseline paragraphs: P26
SOL paragraphs: P26
Korean lines: 53

BASE:

Incredible strength. At this rate, I might get buried standing up.

SOL:

Incredible strength. At this rate, I might be buried standing up.

## H017 (replace)

Baseline paragraphs: P28
SOL paragraphs: P28
Korean lines: 57

BASE:

I drew up every last bit of internal energy in my dantian. The force flooded into every limb. As the spear shaft slowly rose, the Great Warrior’s eyes wavered.

SOL:

I drew up every last bit of internal energy in my dantian. Power flooded through every limb. As my spear shaft slowly rose, the Great Warrior’s eyes wavered.

## H018 (replace)

Baseline paragraphs: P30
SOL paragraphs: P30
Korean lines: 61

BASE:

If it had been whole from the start, maybe. After losing both arms, the Hobgoblin Great Warrior was no longer my match.

SOL:

It might have been different if the creature had been whole, but after losing both arms, the Hobgoblin Great Warrior was no longer a match for me.

## H019 (replace)

Baseline paragraphs: P32
SOL paragraphs: P32
Korean lines: 65

BASE:

In the end, it was the one that backed off first. And from that point, the fight was as good as decided.

SOL:

In the end, it was the first to back off. By then, the fight was as good as over.

## H020 (replace)

Baseline paragraphs: P33
SOL paragraphs: P33
Korean lines: 67

BASE:

I didn’t stop. I thrust the spear out.

SOL:

I pressed forward and thrust my spear.

## H021 (replace)

Baseline paragraphs: P35
SOL paragraphs: P35
Korean lines: 71

BASE:

The movements engraved in my body and head unfurled in a rush.

SOL:

The movements engraved into my body and mind unfolded in rapid succession.

## H022 (replace)

Baseline paragraphs: P36
SOL paragraphs: P36
Korean lines: 73

BASE:

Thrust. Cut. Block and strike with the shaft. Simple on their own, but the order and the angles made countless combinations.

SOL:

Thrust. Cut. Block with the shaft, then strike. Each movement was simple on its own, but changing the order and target created countless combinations.

## H023 (replace)

Baseline paragraphs: P37
SOL paragraphs: P37
Korean lines: 75

BASE:

That was martial arts, as I defined it.

SOL:

That was my definition of martial arts.

## H024 (replace)

Baseline paragraphs: P41
SOL paragraphs: P41
Korean lines: 83

BASE:

Those hooked claws, hard as steel, were cut away. With every step it retreated, a new wound opened and more blood spilled.

SOL:

The hooked claws, once as hard as steel, were sliced away. With every step the creature retreated, another wound opened and more blood spilled.

## H025 (replace)

Baseline paragraphs: P42
SOL paragraphs: P42
Korean lines: 85

BASE:

Then the Hobgoblin Great Warrior’s back hit the wall.

SOL:

Before long, the Hobgoblin Great Warrior’s back struck the wall.

## H026 (replace)

Baseline paragraphs: P44
SOL paragraphs: P44
Korean lines: 89

**Terminology alert:** `가지` → `Go`

BASE:

A C-rank Rare Monster. This thing had seemed impossible to face, but those red eyes had already lost their will to fight a long time ago.

SOL:

A C-rank Rare Monster. A creature that had once seemed impossible to fight. But the will to resist had long since vanished from its red eyes.

## H027 (replace)

Baseline paragraphs: P45
SOL paragraphs: P45
Korean lines: 91

BASE:

“Go. Now.”

SOL:

“Time to go.”

## H028 (replace)

Baseline paragraphs: P47
SOL paragraphs: P47
Korean lines: 95

BASE:

Ghk. With a death rattle, the hulking green body slid down the wall.

SOL:

Ghk. With a final death rattle, the hulking green body slid down the wall.

## H029 (replace)

Baseline paragraphs: P51
SOL paragraphs: P51
Korean lines: 105

BASE:

The System notification rang. I’d thought I’d never hear it or see it again…

SOL:

The System notification rang out. I’d thought I would never hear or see one again…

## H030 (replace)

Baseline paragraphs: P52
SOL paragraphs: P52
Korean lines: 109

BASE:

*A System, out of nowhere?*

SOL:

*The System, out of nowhere?*

## H031 (replace)

Baseline paragraphs: P53
SOL paragraphs: P53
Korean lines: 111

BASE:

I wasn’t the only one confused.

SOL:

But I wasn’t the only one confused.

## H032 (replace)

Baseline paragraphs: P55
SOL paragraphs: P55
Korean lines: 115

BASE:

Team Leader Choi had already taken care of the Priest. His pupils trembled like they’d been hit by an earthquake as he looked from me to the Great Warrior’s corpse and back.

SOL:

Team Leader Choi had already dealt with the Priest. His pupils shook as if an earthquake had struck while he looked back and forth between me and the Great Warrior’s corpse.

## H033 (replace)

Baseline paragraphs: P57
SOL paragraphs: P57
Korean lines: 119

BASE:

Yeah. I’d like to know that myself.

SOL:

Good question. I wanted to know too.

## H034 (replace)

Baseline paragraphs: P61
SOL paragraphs: P61
Korean lines: 131

BASE:

The cigarette in the government official’s mouth dropped.

SOL:

The cigarette dropped from the government official’s mouth.

## H035 (replace)

Baseline paragraphs: P62
SOL paragraphs: P62
Korean lines: 133

**Terminology alert:** `가지` → `Go`

BASE:

He looked born to be a civil servant, and this was anything but good news for him. Sure enough, the way he looked us over was nothing but unease.

SOL:

He looked like a born civil servant, so this couldn’t have been welcome news. Sure enough, his gaze darted anxiously over us.

## H036 (replace)

Baseline paragraphs: P64
SOL paragraphs: P64
Korean lines: 137

BASE:

Team Leader Choi gave the Priest’s staff a weary shake.

SOL:

Team Leader Choi wearily shook the Priest’s staff.

## H037 (replace)

Baseline paragraphs: P69
SOL paragraphs: P69
Korean lines: 147

BASE:

I kicked the Hobgoblin Great Warrior’s greatsword I’d been using like a walking stick. At the dull sound, the official turned.

SOL:

I kicked the Hobgoblin Great Warrior’s greatsword, which I’d been using as a walking stick. The official turned at the dull clang.

## H038 (replace)

Baseline paragraphs: P73
SOL paragraphs: P73
Korean lines: 155

**Terminology alert:** `가지` → `Go`

BASE:

I got the reaction. This had to at least make some kind of sense.

SOL:

I understood his reaction. This was too absurd to make any sense.

## H039 (replace)

Baseline paragraphs: P74
SOL paragraphs: P74
Korean lines: 157

BASE:

The official looked back and forth between us and the gear for a while, then finally nodded with a sad face.

SOL:

The official spent a while looking back and forth between us and the equipment before finally nodding with a miserable expression.

## H040 (replace)

Baseline paragraphs: P77
SOL paragraphs: P77
Korean lines: 163

BASE:

Im Kkeokjeong had come to at some point. I was holding him up. He’d taken serious injuries—at least five broken bones.

SOL:

Im Kkeokjeong had regained consciousness at some point. I was supporting him, and he’d suffered severe injuries, including at least five broken bones.

## H041 (replace)

Baseline paragraphs: P78
SOL paragraphs: P78
Korean lines: 165

BASE:

“Call a healer. A pretty unnie.”

SOL:

“Call us a healer. A pretty one.”

## H042 (replace)

Baseline paragraphs: P79
SOL paragraphs: P79
Korean lines: 167

BASE:

The E-rank trio dropped onto the ground as well.

SOL:

The E-rank trio collapsed onto the ground as well.

## H043 (replace)

Baseline paragraphs: P80
SOL paragraphs: P80
Korean lines: 169

BASE:

“And potions! Potions! The good stuff!”

SOL:

“And potions! Potions! Give us the good stuff!”

## H044 (replace)

Baseline paragraphs: P81
SOL paragraphs: P81
Korean lines: 171

BASE:

“Ah, for fuck’s sake, how did you people even manage this Gate?!”

SOL:

“For fuck’s sake, how did you people even manage this Gate?!”

## H045 (replace)

Baseline paragraphs: P82
SOL paragraphs: P82
Korean lines: 173

BASE:

“Our gear’s all smashed too, huh? What are you going to do about this?!”

SOL:

“All our gear’s smashed too! Huh? What are you going to do about this?!”

## H046 (replace)

Baseline paragraphs: P83
SOL paragraphs: P83
Korean lines: 175

BASE:

Do about it? The government would cover everything.

SOL:

What did they think he was going to do? The government would compensate them for everything.

## H047 (replace)

Baseline paragraphs: P84
SOL paragraphs: P84
Korean lines: 177

BASE:

There was an actual law for this: every item used in the Gate, plus medical costs and compensation.

SOL:

There was an actual law requiring it to cover every item consumed inside the Gate, along with medical expenses and compensation.

## H048 (replace)

Baseline paragraphs: P85
SOL paragraphs: P85
Korean lines: 179

BASE:

So it was a show. Squeeze out a little extra.

SOL:

In other words, this was all a show to squeeze out a little more.

## H049 (replace)

Baseline paragraphs: P86
SOL paragraphs: P86
Korean lines: 181

BASE:

*Tsk. Even so. People are watching.*

SOL:

*Tsk, tsk. Even so, people are watching.*

## H050 (replace)

Baseline paragraphs: P87
SOL paragraphs: P87
Korean lines: 183

BASE:

That was when I felt a stinging look from beside me.

SOL:

That was when I felt a piercing stare from beside me.

## H051 (replace)

Baseline paragraphs: P89
SOL paragraphs: P89
Korean lines: 187

BASE:

Team Leader Choi.

SOL:

It was Team Leader Choi.

## H052 (replace)

Baseline paragraphs: P91
SOL paragraphs: P91
Korean lines: 191

BASE:

He was looking at me, confused. More precisely, at the hand holding my dagger.

SOL:

He stared at me in confusion. More precisely, at the hand holding my dagger.

## H053 (replace)

Baseline paragraphs: P93
SOL paragraphs: P93
Korean lines: 195

BASE:

The dagger, charged with internal energy, raked long lines through seven-year leather armor until it was too wrecked to use.

SOL:

The dagger, charged with internal energy, carved long gashes through my seven-year-old leather armor until it was too badly damaged to use.

## H054 (replace)

Baseline paragraphs: P94
SOL paragraphs: P94
Korean lines: 197

BASE:

“...”

SOL:

“…”

## H055 (replace)

Baseline paragraphs: P95
SOL paragraphs: P95
Korean lines: 199

BASE:

“...”

SOL:

“…”

## H056 (replace)

Baseline paragraphs: P97
SOL paragraphs: P97
Korean lines: 203

BASE:

I sighed hard enough to cave in the ground.

SOL:

I heaved a sigh deep enough to sink the ground.

## H057 (replace)

Baseline paragraphs: P98
SOL paragraphs: P98
Korean lines: 205

BASE:

“The Hobgoblin Great Warrior. It was really strong. I made it out alive, but every piece of gear I had is ruined.”

SOL:

“That Hobgoblin Great Warrior was incredibly strong. I escaped with my life, but every piece of equipment I owned was destroyed.”

## H058 (replace)

Baseline paragraphs: P99
SOL paragraphs: P99
Korean lines: 207

BASE:

“...”

SOL:

“…”

## H059 (replace)

Baseline paragraphs: P100
SOL paragraphs: P100
Korean lines: 209

BASE:

“The armor that protected me for seven years. And now even the spear.”

SOL:

“The armor that protected me for seven years. Now even my spear…”

## H060 (replace)

Baseline paragraphs: P102
SOL paragraphs: P102
Korean lines: 213

BASE:

Right then, I pressed down steadily on the middle of the spear with one foot.

SOL:

At that moment, I planted one foot on the middle of the spear and slowly pressed down.

## H061 (replace)

Baseline paragraphs: P103
SOL paragraphs: P103
Korean lines: 215

BASE:

Under the majesty of a three-digit Strength stat, the iron spear bent like a stick of taffy.

SOL:

Before the majesty of my three-digit Strength stat, the iron spear bent like taffy.

## H062 (replace)

Baseline paragraphs: P104
SOL paragraphs: P104
Korean lines: 217

BASE:

“Looks perfectly fine? This?”

SOL:

“Perfectly fine? This does?”

## H063 (replace)

Baseline paragraphs: P105
SOL paragraphs: P105
Korean lines: 219

BASE:

“...”

SOL:

“…”

## H064 (replace)

Baseline paragraphs: P108
SOL paragraphs: P108
Korean lines: 227

BASE:

Team Leader Choi was called away by the officials dispatched to take charge, and the other four were moved to an ambulance for treatment.

SOL:

As the person in charge, Team Leader Choi was called away by the dispatched officials, while the other four were taken to an ambulance for treatment.

## H065 (replace)

Baseline paragraphs: P109
SOL paragraphs: P109
Korean lines: 231

BASE:

Instead of the musty break room, I was left alone in a wide, comfortable office. Now that the moment had actually come, my heart started pounding.

SOL:

Instead of the musty break room, I was left alone in a spacious, comfortable office. Now that the moment had finally arrived, my heart began to pound.

## H066 (replace)

Baseline paragraphs: P111
SOL paragraphs: P111
Korean lines: 235

BASE:

At that half-doubtful call, the System answered.

SOL:

The System answered my doubtful summons.

## H067 (replace)

Baseline paragraphs: P113
SOL paragraphs: P113
Korean lines: 239

BASE:

> **System**
>
> **Status Window**
>
> **Lv. 33 Jin Taekyung**
>
> **Job:** First Rate Martial Artist
>
> **Fame:** 0
>
> **Titles:** 2 (Title effects active)
>
> - Novice Trainee (Training speed +10%)
> - Gambler (Combat-related stats +10% in one-on-one matches)
>
> **Strength:** 120  **Stamina:** 125
>
> **Agility:** 121  **Intelligence:** 20
>
> **Charm:** 20  **Internal Energy:** 15 years
>
> **Remaining Points:** 30
>
> Synchronization has been completed. Changes have occurred to Titles and Fame.

SOL:

> **System**
>
> **Status Window**
>
> **Lv. 33 Jin Taekyung**
>
> **Job:** First Rate Martial Artist
>
> **Fame:** 0
>
> **Titles:** 2 (Title effects active)
>
> - Novice Trainee (Training speed +10%)
> - Gambler (Combat-related stats +10% in one-on-one matches)
>
> **Strength:** 120  **Stamina:** 125
>
> **Agility:** 121  **Intelligence:** 20
>
> **Charm:** 20  **Internal Energy:** 15 years
>
> **Remaining Points:** 30
>
> Synchronization is complete. Titles and Fame have changed.

## H068 (replace)

Baseline paragraphs: P114
SOL paragraphs: P114
Korean lines: 243

BASE:

“Fuck. It really came up.”

SOL:

“Fuck. It really appeared.”

## H069 (replace)

Baseline paragraphs: P115
SOL paragraphs: P115
Korean lines: 247

BASE:

I stared at the Status Window, blank, then noticed something off.

SOL:

I stared blankly at the Status Window for a moment before noticing something strange.

## H070 (replace)

Baseline paragraphs: P117
SOL paragraphs: P117
Korean lines: 251

BASE:

The first things that jumped out were Fame reset to zero and two missing Titles. The last line gave me a rough idea why.

SOL:

The first things that jumped out were my Fame, which had been reset to zero, and the two missing Titles. The final line gave me a rough idea why.

## H071 (replace)

Baseline paragraphs: P119
SOL paragraphs: P119
Korean lines: 255

BASE:

All five hundred Fame I’d stacked had been earned as a martial artist in Murim. The Title that had vanished, *Scion of a Prestigious Family*, made sense the same way.

SOL:

All five hundred Fame I’d accumulated had been earned as a martial artist in Murim. The missing Title, *Scion of a Prestigious Family*, made sense for the same reason.

## H072 (replace)

Baseline paragraphs: P120
SOL paragraphs: P120
Korean lines: 257

BASE:

*Well, I’m not some prestigious family’s son here. And I’m sure as hell no Hidden Dragon.*

SOL:

*Well, I’m not the son of some prestigious family here. And I’m sure as hell no Hidden Dragon.*

## H073 (replace)

Baseline paragraphs: P121
SOL paragraphs: P121
Korean lines: 259

BASE:

If I were getting a Title here, it’d obviously be something like *Scion of the Common Folk*, or *Earthworm* instead of Sleeping Dragon.

SOL:

If I earned a Title here, it would probably be something like *Scion of the Common Folk*. Or *Earthworm* instead of *Hidden Dragon*.

## H074 (replace)

Baseline paragraphs: P122
SOL paragraphs: P122
Korean lines: 261

BASE:

“How much else changed?”

SOL:

“What else changed?”

## H075 (replace)

Baseline paragraphs: P123
SOL paragraphs: P123
Korean lines: 263

BASE:

For about ten minutes I turned every System function on and off and tested them. The only thing that had changed was the Status Window.

SOL:

For the next ten minutes or so, I opened, closed, and tested every System function. The Status Window was the only thing that had changed.

## H076 (replace)

Baseline paragraphs: P124
SOL paragraphs: P124
Korean lines: 265

**Terminology alert:** `퀘스트` → `Quest`

BASE:

No. There was one more. The Quest Window.

SOL:

No. There was one more.

## H077 (insert)

Baseline paragraphs: after P124
SOL paragraphs: P125
Korean lines: 265

BASE:

*(empty)*

SOL:

The Quest Window.

## H078 (replace)

Baseline paragraphs: P126
SOL paragraphs: P127
Korean lines: 273

**Terminology alert:** `초식` → `form`

BASE:

> **System**
>
> **Quest**
>
> **Traitor**
>
> You have discovered the traitor’s identity. Rejoin the main force, inform them that there is a traitor, and lead the battle to victory!
>
> **Grade:** Peak
>
> **Restriction:** Jin Taekyung
>
> **Mission:** Punish the traitor (Incomplete)
>
> Battle victory (Incomplete)
>
> **Reward:** Vast EXP and Fame
>
> Valuable iron chest
>
> **Failure:** Death

SOL:

> **System**
>
> **Quest**
>
> **Traitor**
>
> You have discovered the traitor’s identity. Rejoin the main force, warn them of the traitor, and lead the battle to victory!
>
> **Grade:** Peak
>
> **Restriction:** Jin Taekyung
>
> **Mission:** Eliminate the traitor (Incomplete)
>
> Win the battle (Incomplete)
>
> **Reward:** Vast EXP and Fame
>
> Valuable iron chest
>
> **Failure:** Death

## H079 (replace)

Baseline paragraphs: P127
SOL paragraphs: P128
Korean lines: 275

**Terminology alert:** `가지` → `Go`

BASE:

The Chain Quest I’d gotten after taking down Gwak Jun and the assassins. My thoughts tangled.

SOL:

It was the Chain Quest I’d received after defeating Gwak Jun and the assassins. My thoughts grew tangled.

## H080 (replace)

Baseline paragraphs: P128
SOL paragraphs: P129
Korean lines: 277

BASE:

*Go back? To Murim?*

SOL:

*Does it want me to go back? To Murim?*

## H081 (replace)

Baseline paragraphs: P129
SOL paragraphs: P130
Korean lines: 279

BASE:

I knew it now. The Murim I’d been through wasn’t an illusion, and it wasn’t a game. A wildly unrealistic story, sure, but Murim was probably…

SOL:

I knew now. The Murim I’d experienced wasn’t an illusion or a game. As wildly unrealistic as it sounded, Murim was probably…

## H082 (replace)

Baseline paragraphs: P131
SOL paragraphs: P132
Korean lines: 283

BASE:

I turned at the sudden sound. Team Leader Choi was standing in the doorway.

SOL:

I turned at the sudden sound. Team Leader Choi stood in the doorway.

## H083 (replace)

Baseline paragraphs: P133
SOL paragraphs: P134
Korean lines: 287

BASE:

Looked like the talk had wrapped up. I folded the thought away and stood.

SOL:

It seemed the discussion had wrapped up. I put the thought aside and rose.

## H084 (replace)

Baseline paragraphs: P135
SOL paragraphs: P136
Korean lines: 291

BASE:

“They should be on their way to the hospital by now. They said about a week of admission. You don’t need to worry.”

SOL:

“They should be on their way to the hospital by now. They’ll need to stay there for about a week. You don’t need to worry.”

## H085 (replace)

Baseline paragraphs: P136
SOL paragraphs: P137
Korean lines: 293

BASE:

I wasn’t particularly worried. Combat-type Hunter bodies, plus potions and healing magic—they’d be up soon enough.

SOL:

I wasn’t particularly worried. Combat-type Hunters had sturdy bodies, and they’d received potions and healing magic. They would be back on their feet soon enough.

## H086 (replace)

Baseline paragraphs: P139
SOL paragraphs: P140
Korean lines: 303

BASE:

“They’ll contact us again in a few days. An investigator will probably come see you, too.”

SOL:

“They’ll contact us again within a few days. An investigator will probably visit you as well.”

## H087 (replace)

Baseline paragraphs: P141
SOL paragraphs: P142
Korean lines: 309

BASE:

Not a type I wanted to run into again. But that wasn’t the answer I’d wanted, so I asked again.

SOL:

Not the sort of person I ever wanted to deal with again. But that wasn’t the answer I wanted, so I asked once more.

## H088 (replace)

Baseline paragraphs: P142
SOL paragraphs: P143
Korean lines: 311

BASE:

“And the rest?”

SOL:

“What about the other matter?”

## H089 (replace)

Baseline paragraphs: P144
SOL paragraphs: P145
Korean lines: 315

BASE:

Team Leader Choi looked at me with an odd expression. Like he was studying some strange animal.

SOL:

Team Leader Choi regarded me with a strange expression, like he was examining some fascinating animal.

## H090 (replace)

Baseline paragraphs: P146
SOL paragraphs: P147
Korean lines: 319

BASE:

I didn’t want my strength out in the open. Even less now, when I still barely knew why the System was showing up in reality, or what it had to do with Murim.

SOL:

I didn’t want my strength exposed. Especially not now, when I still had no idea why the System had appeared in reality or how any of this was connected to Murim.

## H091 (replace)

Baseline paragraphs: P147
SOL paragraphs: P148
Korean lines: 321

BASE:

An F-rank Hunter killing a C-rank Rare Monster alone was unheard of. A needle in a bag eventually pokes through. I’d lived the opposite kind of life, nothing sticking out, so I had to be careful.

SOL:

An F-rank Hunter single-handedly killing a C-rank Rare Monster was unprecedented. A needle in a bag would eventually poke through, but I’d spent my whole life being the opposite of that—never standing out at all. I couldn’t help being cautious.

## H092 (replace)

Baseline paragraphs: P148
SOL paragraphs: P149
Korean lines: 327

BASE:

So I asked him.

SOL:

So I had asked him:

## H093 (replace)

Baseline paragraphs: P149
SOL paragraphs: P150
Korean lines: 329

BASE:

“Could you keep what concerns me a secret?”

SOL:

“Could you keep what happened with me a secret?”

## H094 (replace)

Baseline paragraphs: P150
SOL paragraphs: P151
Korean lines: 331

BASE:

From Team Leader Choi’s side, it had to be a hard ask. I was telling him to lie in his statement for someone he’d only just met.

SOL:

It must have been a difficult request from Team Leader Choi’s perspective. I was asking him to give a false statement for someone he’d only just met.

## H095 (replace)

Baseline paragraphs: P153
SOL paragraphs: P154
Korean lines: 337

BASE:

His heavy voice made my chest sink with it—and then:

SOL:

His grave voice made my heart sink.

## H096 (replace)

Baseline paragraphs: P154
SOL paragraphs: P155
Korean lines: 339

BASE:

“Let’s get some food.”

SOL:

Then he said, “Let’s get something to eat.”

## H097 (replace)

Baseline paragraphs: P157
SOL paragraphs: P158
Korean lines: 345

BASE:

Team Leader Choi held out his wrist. A glittering Magic Gem electronic watch pointed to two in the afternoon.

SOL:

Team Leader Choi held out his wrist. A gleaming Magic Gem digital watch showed two in the afternoon.

## H098 (replace)

Baseline paragraphs: P158
SOL paragraphs: P159
Korean lines: 347

**Terminology alert:** `가지` → `Go`

BASE:

“It’s an N Company piece carved from a whole C-rank Magic Gem. It’s got enhancement magic, so in an emergency you can even use it as a shield—”

SOL:

“It’s an N Company model carved from a solid C-rank Magic Gem. It’s enchanted with reinforcement magic, so in an emergency, you can even use it as a shield—”

## H099 (replace)

Baseline paragraphs: P159
SOL paragraphs: P160
Korean lines: 349

**Terminology alert:** `가지` → `Go`

BASE:

*So that’s where he was going.*

SOL:

*So that’s what he meant.*

## H100 (replace)

Baseline paragraphs: P161
SOL paragraphs: P162
Korean lines: 353

BASE:

Impossible man to read.

SOL:

The man was impossible to read.

## H101 (replace)

Baseline paragraphs: P162
SOL paragraphs: P163
Korean lines: 355

BASE:

“Let’s get some meat. You like Hanwoo?[^1]”

SOL:

“Let’s get some meat. Do you like Hanwoo?[^1]”

## H102 (replace)

Baseline paragraphs: P165
SOL paragraphs: P166
Korean lines: 361

BASE:

And rich, too.

SOL:

And he was rich too.

## H103 (replace)

Baseline paragraphs: P166
SOL paragraphs: P167
Korean lines: 363

BASE:

“I never get to eat it.”

SOL:

“I can never get enough of it.”

## H104 (replace)

Baseline paragraphs: P167
SOL paragraphs: P168
Korean lines: 365

BASE:

Not just any beef. Hanwoo. Top-grade Hanwoo at that.

SOL:

This wasn’t just any beef. It was Hanwoo. Top-grade Hanwoo, at that.

## H105 (replace)

Baseline paragraphs: P168
SOL paragraphs: P169
Korean lines: 367

BASE:

System or whatever—first I had to fill a hungry stomach.

SOL:

System or whatever—first, I needed to fill my empty stomach.
