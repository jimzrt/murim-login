...
SOL paragraphs: P11
Korean lines: 27
Terminology alert: `무림` → `Murim` present in BASE, absent from SOL.
...
SOL paragraphs: P73
Korean lines: 161
Terminology alert: `무공` → `martial arts` present in BASE, absent from SOL.
...
### H071 — insert
Baseline paragraphs: after P136
SOL paragraphs: P135
Korean lines: 297
...
SOL paragraphs: P149
Korean lines: 325
Terminology alert: `풍양` → `Pung Yang` present in BASE, absent from SOL.
...
SOL paragraphs: after P150
Korean lines: 329
Terminology alert: `진태경` → `Jin Taekyung` present in BASE, absent from SOL.
...
SOL paragraphs: P163
Korean lines: 355
Terminology alert: `적풍단주` → `Red Wind Band Leader` present in BASE, absent from SOL.
...
SOL paragraphs: P171
Korean lines: 373
Terminology alert: `절정` → `Peak` present in BASE, absent from SOL.
...
SOL paragraphs: P174
Korean lines: 381
Terminology alert: `진무경` → `Jin Mukyung` present in BASE, absent from SOL.
...
SOL paragraphs: after P180
Korean lines: 387
Terminology alert: `잠력단` → `Temporary Strength Pill` present in BASE, absent from SOL.
...
Return exactly the JSON object required by the adjudicator brief, with chapter set to 115 and exactly one decision for each of the 106 hunks.
